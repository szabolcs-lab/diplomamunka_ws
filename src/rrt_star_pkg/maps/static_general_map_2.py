...
def main():
    ...
    # Mentés CSV-be
    np.savetxt("occupancy_grid_2.csv", grid, fmt="%d", delimiter=",")

    # SDF generálása
    sdf_text = generate_sdf(walls, grid_size_x, grid_size_y)
    with open("custom_world_2.sdf", "w") as f:
        f.write(sdf_text)

    print("Generált occupancy_grid_2.csv és custom_world_2.sdf fájlokat.")
